...
def translation(domain, localedir=None, languages=None, _class=None,
    ...
    if languages:
        ...
        locale_suffix = os.path.join(
            'LC_MESSAGES', os.extsep.join((domain, 'mo')))
...
_N = translations.ngettext
assert isinstance(
    translations, LazyInstance) and translations._li_instance is None
...
class ChoiceHighlighters(
        collections.namedtuple('ChoiceHighlightersBase',
                               ('shorthand', 'default'))
):
    ...
    def from_termcaps(cls, shorthand_args, default_args):
        return cls(*map(
            fpartial(cls._call_with_positional_or_keyword_args,
                     cls.from_termcap),
...
class Choices(collections.ChainMap):
    ...
    def __init__(self, *choices, default=None, use_shorthands=None, joiner='/',
        ...
        if use_shorthands is None:
            ...
            use_shorthands = getattr(
                use_shorthands, '__contains__', None) or bool
    ...
    def ask(self, question, sep='  ', *args, **kwargs):
        ...
        if isinstance(answer, str):
            answer = self.get(normalize_casefold(
                answer)) if answer else self.default
        return answer
