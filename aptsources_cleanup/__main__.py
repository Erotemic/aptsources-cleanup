...
def main(args=None):
    ...
    if args.debug_import_fail:
        ...
        import_check('aptsources.sourceslist', 'apt',
                     None, args.debug_import_fail)
    ...
    def _format_actions_usage(self, actions, groups):
        return (
            super()._format_actions_usage(
                tuple(filterfalse(
                    methodcaller(
                        isinstance, (argparse._HelpAction, VersionAction)),
                    actions)),
                groups))
    ...
    def __call__(self, parser, namespace, values, option_string):
        ...
        if version is None:
            if __package__ and __name__ == '__main__':
                version = getattr(
                    sys.modules[__package__], '__version__', None)
            else:
...
def parse_args(args):
    ...
    translations.add_fallback(
        DictTranslations(ID_DESCRIPTION=prefix(aptsources_cleanup.__doc__, '\n\n\n', reverse=True).strip()))
    ...
    ap.add_argument('--equivalent-schemes', metavar='SCHEMES',
                    type=fpartial(EquivalenceRelation.parse,
                                  settype="ordered"),
    ...
    if duplicates:
        for dupe_set in duplicates:
            ...
            for dupe in dupe_set:
                ...
                stdout.print(
                    _("I disabled all but the first entry."), end="\n\n")
                dupe.disabled = True
        ...
        stdout_indent2.print_all(
            map(str, itertools.chain(*duplicates)), sep='\n')
...
def sort_dupe_set_by_scheme_class(eqclasses, dupe_set):
    if eqclasses and dupe_set:
        ...
        if schemes_class and getattr(schemes_class, "index", None) is not None:
            dupe_set.sort(key=lambda se: schemes_class.index(
                se.parsed_uri.scheme))
    return dupe_set
